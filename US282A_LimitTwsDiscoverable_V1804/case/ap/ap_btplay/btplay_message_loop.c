...
void __section__(".text.autoplay") btplay_autoplay_deal(void)
{
    ...
    else if (g_bt_stack_cur_info.rmt_dev[g_bt_stack_cur_info.a2dp_active_id].dev_type == TWS_DEV)
    {
        return;
    }
#endif
...
void __section__(".bank_2")det_auto_play_flag(void)
{
    if((g_customer_state&S3WAKE_ONOFF_PLAY) != 0)
	{
		    ...
		    // g_customer_state&= (~S3WAKE_ONOFF_PLAY) ;
		     // PRINTD_INFO("UPAUTOPLAY11");
		     if((g_btplay_cur_info.status != BTPLAY_PLAY) && (g_btplay_autoplay_flag == 0))
		     {
   ...
   /* if((g_customer_state&S3WAKE_ONOFF_PLAY) != 0)
    ...
    while (1)
    {
        ...
        if((g_app_info_state_all.g_hp_led_state == LED_CONNECTED) 
            || (g_app_info_state_all.g_hp_led_state == LED_PLAY)
            ||  (g_app_info_state_all.g_hp_led_state == LED_PAUSE))
        {
            if(g_btplay_cur_info.status == BTPLAY_PLAY)
            {
                if(g_app_info_state_all.g_hp_led_state != LED_PLAY)
                {
                    set_led_state(LED_PLAY);
                }
            }
            else
            {
                if(g_app_info_state_all.g_hp_led_state != LED_PAUSE)
                {
                    set_led_state(LED_PAUSE);
                }
            }
        }
