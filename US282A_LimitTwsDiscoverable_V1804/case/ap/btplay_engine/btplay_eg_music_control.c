...
void a2dp_decode_init(uint8 media_type)
{
    ...
    if (mmm_init_ret == 0)
    {
        ...
        {
            //左声道
			//#ifdef ENABLE_OUTPUT_CH_DET_BY_IO
			...
			else if((act_readl(GPIOBDAT) & (1<<8)) == 0)
			{
			   mmm_bp_cmd(mp_handle, MMM_BP_SET_OUTPUT_CHANNEL,
        ...
        {
            mmm_bp_cmd(mp_handle, MMM_BP_SET_OUTPUT_CHANNEL,
                    (unsigned int) s_output_channel[2]);
        ...
        if (result == 0)
        {
            ...
            if(g_p_bt_stack_cur_info->dev_role != 0)
            {	
            	 if (tws_adjfreq_timer_id != -1)
               {
                  kill_app_timer(tws_adjfreq_timer_id);
                  tws_adjfreq_timer_id = -1;
               }               
               tws_adjfreq_timer_id = set_single_shot_app_timer(APP_TIMER_ATTRB_CONTROL,
                2000, tws_adjust_freq);
            }
#endif            
