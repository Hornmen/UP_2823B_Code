...
bool com_need_cont_dck_deal(uint8 key_count)
{
    ...
    {
        if((g_key_infor.tck_flag == TRUE)&&
            (g_key_infor.status > KEY_STATUS_DOWN_2ND))
        {
            g_key_infor.count -= (g_key_infor.status - 5);
            g_key_infor.status = KEY_STATUS_DOWN_2ND;
            return TRUE;
        }
        else
...
bool com_phy_key_fsm(input_gui_msg_t *input_msg)
{
    ...
    if (key_msg.key_status == KEY_PHY_UP)
    {
		 ...
		 {
		      g_key_infor.status = KEY_STATUS_NULL;   //add2018.4.29 防止开机后按键松开触发事件
		      g_customer_state&= (~CUSTOMER_POWERONOFF) ;
			  act_writel(act_readl(ONOFF_KEY)|0x30, ONOFF_KEY);
			  com_flush_rtc_reg();
		  	  act_writel(act_readl(ONOFF_KEY)| (1<<ONOFF_KEY_ONOFF_RST_EN), ONOFF_KEY);
		      com_flush_rtc_reg();
			  PRINTD_INFO("POWERON KeyUP");
    ...
    switch (g_key_infor.status)
    {
        ...
        if ((g_key_infor.dck_fail == TRUE) || 
            (((g_key_infor.dck_map & (uint32) (1 << key_log_id)) == 0) &&
            ((g_key_infor.tck_map & (uint32) (1 << key_log_id)) == 0)))
        { //不需要分析双击按键，读取按键消息并返回
        ...
        if (key_log_id == g_key_infor.dck_val)
        {
            if ((key_msg.key_status == KEY_PHY_UP)&&
                ((g_key_infor.tck_map & (uint32) (1 << key_log_id)) != 0) &&
                (g_key_infor.tck_flag == FALSE))
            {
                g_key_infor.status = KEY_STATUS_UP_2ND;
                g_key_infor.tck_flag = TRUE;
                g_key_infor.dck_timestamp = (uint16) sys_get_ab_timer();
            }
            else if (key_msg.key_status == KEY_PHY_UP)
            {
                uint8 i;
        ...
        break;
        case KEY_STATUS_UP_2ND:
        g_key_infor.count++;
        if (key_log_id == g_key_infor.dck_val)
        {
            g_key_infor.status = KEY_STATUS_DOWN_3ND;
