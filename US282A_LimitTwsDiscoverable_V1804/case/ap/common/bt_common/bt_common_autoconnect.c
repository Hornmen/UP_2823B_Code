...
/* 
	1、
		A、B、C三个耳机，名称相同，高三字节相同；
    	A与B先组队，B作为主耳，A关机；
    	B和C组队，C做主耳，C关机；
    	A、B重启，当B回连C过程中，A回连上B,那么B应该停止对C的回连。

	2、
		A、B、C三个耳机，A和B的高三字节相同，C与他们的不同，A、B、C名称都相同。
	A和B组队，B作为主耳，然后用C向B发起组队，这个时候C可以组上B，B认为C的dev_type
	是dev_tws，但是由于高三字节不同，把C的配对信息存入到手机配对列表中，因此
	当A、B重启，A、B回连上后，B还会回连C，由于C的dev_type是dev_tws，当B回连C
	失败后，会在A2dpStreamDoneInd函数中，清除dev_role，使得B的tws信息
	阻塞，无法正常发出和接收。并且主耳会播报蓝牙断开，等待连接的tts：

#ifdef ENABLE_TRUE_WIRELESS_STEREO
    if (dev->rmt_dev.dev_info.dev_type == TWS_DEV)
    {
        dev->a2dp_source.is_source = 0;
        g_p_bt_stack_info->dev_role = 0;
    }
#endif

    //主箱与手机断开连接的播报
    if (g_p_bt_stack_info->dev_role == 1)
    {
        g_p_bt_stack_info->phonetomastercon=0;
        //g_p_bt_stack_info->pair=0;
        if(dev->rmt_dev.dev_info.dev_type == TWS_DEV)
        {
            g_p_bt_stack_info->pair=0;
            g_p_bt_stack_info->phonetomasterdis=0;

            //避免挂断电话后，回连副箱上播蓝牙连接成功
            g_p_bt_stack_info->tws_hfp_con_flag=0;
        }
    }
*/
	if ( (g_bt_stack_cur_info.dev_role==TWS_MASTER) 
		&& (g_bt_stack_cur_info.num_connected == 1))
	{
		for(j = 0;j < g_btmanager_gl_var.support_dev_num;j++)
		{
			if (g_bt_auto_connect_ctrl.dev_info[j].dev_type == TWS_DEV)
			{
				g_bt_auto_connect_ctrl.dev_info[j].conn_flag = 0;
				g_bt_auto_connect_ctrl.dev_info[j].connect_cnt = 0; 					   
			}	 
		}
	}
...
void com_btmanager_autoconnect(void)
{
	...
	/*if(g_btmanager_gl_var.tws_dev_null == 1)
	{
	    g_btmanager_gl_var.tws_dev_null = 2;
	...
	/* if(g_btmanager_gl_var.tws_dev_null == 0)
	{
	     
	     g_btmanager_gl_var.contotws_timer_cnt++;
		if(g_btmanager_gl_var.contotws_timer_cnt>= 18)
		{
		      g_btmanager_gl_var.contotws_timer_cnt = 0;
		      g_btmanager_gl_var.tws_dev_null = 1;
			 if (g_app_info_vector[APP_TYPE_BTSTACK].used != 0)
		   	  {
           	   if((g_bt_stack_cur_info.dev_role==0)&&(g_bt_stack_cur_info.conn_status==CONN_STATUS_WAIT_PAIR)
        	     &&(g_btmanager_gl_var.support_dev_num>1))
