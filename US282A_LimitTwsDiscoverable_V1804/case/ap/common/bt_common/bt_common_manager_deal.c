...
void set_led_state(led_status_e led_state)__FAR__;
...
/*
 0.close discoverable              for slave: 不可见、不可连接  for other: 只是关闭可见性
 1.open discoverable、connected    打开可见性、可连接性
 2.close discoverable、connected   关闭可见性,可连接性
 3.open limit discoverable         只在tws组队时使用
*/
void com_set_discoverable(uint8 mode)
{
	msg_apps_t msg;
	if (g_btmanager_gl_var.discoverable_flag != mode)
	{
		g_btmanager_gl_var.discoverable_flag = mode;
       //  PRINT_INFO("up tws connected");
...
void __section__(".rcode") com_btmanager_discoverable_handle(void)
{
    ...
    {    
      ...
      else  // 一个连接
      {
		...
		// 未连接上手机的主箱应该打开可见性
		else if (g_bt_stack_cur_info.dev_role == TWS_MASTER)
		{
		    
			com_set_discoverable(1);
		}
		// 手机连接上即关闭可见性
		if ((g_bt_stack_cur_info.dev_role == NORMAL_DEV) && (g_bt_stack_cur_info.hfp_established == 1)) 
		{
			com_set_discoverable(0);
		}		       
      }		            	      
    ...
    {
        if ((act_readl(GPIOBDAT) & (1<<9)) == 0)   
    	{
		    if((g_customer_state&CUSTOMER_TWSPAIR_FLAG) == 0)
		    {
		      // PRINTD_INFO("UP SLAVE DISCOVERABLE1");
				com_set_discoverable(1);
		    }
    	}
    	else 
    	{ 
    	    if (g_bt_stack_cur_info.tws_pair_status != 1) 
            {
                com_set_discoverable(1);
				//PRINTD_INFO("UP MASTER DISCOVERABLE1");
				if((g_customer_state&CUSTOMER_TWSPAIR_FLAG) != 0)
				{
				     //PRINTD_INFO("UP MASTER TWS FINISHED");
...
void __section__(".rcode") com_btmanager_hfp_handle(void)
{
    ...
    if (g_this_app_info->app_id != APP_ID_BTCALL)
    {
        for (i = 0; i < g_btmanager_gl_var.support_dev_num; i++)
        {
            ...
            {
                if((g_bt_stack_cur_info.rmt_dev[i].hfp_status==HFP_STATUS_INCOMING)
                    ||(g_bt_stack_cur_info.rmt_dev[i].hfp_status==HFP_STATUS_OUTGOING))
                {
                    set_led_state(LED_CALLING);
                 }
    ...
    {
        if ((g_btmanager_gl_var.last_hfp_status[g_bt_stack_cur_info.hfp_active_id] == HFP_STATUS_INCOMING)
                && (g_bt_stack_cur_info.rmt_dev[g_bt_stack_cur_info.hfp_active_id].hfp_status != HFP_STATUS_INCOMING))
        {
...
//#endif
    {
        for (i = 0; i < g_btmanager_gl_var.support_dev_num; i++)
        {
            ...
            if ((g_bt_stack_cur_info.rmt_dev[i].sniff_status == 0)
                    ...
                    && (((g_bt_stack_cur_info.dev_role == 1) && (g_bt_stack_cur_info.rmt_dev[i].dev_type == TWS_DEV))
...
void __section__(".bank") com_btmanager_fast_discover_timer_proc(void)
{
	  ...
	  if(g_fast_discoverable_cnt>=50)
	  {
		  ...
		  libc_print("GPIOB",act_readl(GPIOBDAT),2);
          if ((act_readl(GPIOBDAT) & (1<<9)) == 0)  //区分主副耳副耳处理
          {
        	      //时间到切到slow Discoverable
			 if((g_customer_state&CUSTOMER_TWSPAIR_FLAG) != 0)  //组队时间到切到CONNECTING
			 {
			     //PRINTD_INFO("UP SLAVE TWS FINISHED");
			     g_customer_state&= (~CUSTOMER_TWSPAIR_FLAG);
				 set_led_state(LED_CONNECTING);
				 g_tws_discoverable_flag = 0;
			 }
          }
...
void __section__(".bank") com_btmanager_fast_discover_deal(uint8 type)
{
      ...
      PRINT_INFO("slave fast");
	       //send message to bt stack to change back san time
	       com_btmanager_set_scan_mode(1);   //进入fast discoverabl 
	   }
	   else 
	   {
	   	  kill_app_timer(g_fast_discoverable_timer);
...
void com_btmanager_conn_status_change_handle(bool init_flag)
{
    ...
    switch (g_bt_stack_cur_info.conn_status)
    {
        ...
        if ((act_readl(GPIOBDAT) & (1<<9)) == 0)   //区分主副耳副耳进入组队的处理
        {
			if((g_customer_state&CUSTOMER_TWSPAIR_FLAG) != 0)
			{
				if(g_tws_discoverable_flag== 0)
				{   
				    PRINTD_INFO("UP SLAVE TWS START");
					kill_app_timer(g_fast_discoverable_timer);
                	g_fast_discoverable_timer = -1;
					g_tws_discoverable_flag = 1;
				}
			}
        }
