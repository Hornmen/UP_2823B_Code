...
/*
*src_flag:0表示搜索到设备以snk角色去连接远端设备
*src_flag:1表示搜索到设备以src角色去连接远端设备
*/
int com_btmanager_force_unlink(uint8 src_flag)
{
    msg_apps_t msg;
            ...
            && (g_bt_stack_cur_info.conn_status == CONN_STATUS_WAIT_PAIR))
    {
        ...
        msg.content.data[1] = src_flag;
        //指定下层tws 搜索次数
        msg.content.data[2] = 15;
        //发送同步消息
        send_sync_msg_btmanager(NULL, &msg, NULL, 0);
    }
    else
...
int com_btmanager_force_unlink_tws(void)
{
	msg_apps_t msg;
	
    if ((g_app_info_vector[APP_TYPE_BTSTACK].used == 1) && (g_bt_stack_cur_info.conn_status == CONN_STATUS_LINKED))
    {
...
int com_btmanager_clearpairlist(void)
{
    ...
    if ( g_app_info_vector[APP_TYPE_BTSTACK].used == 1 )
    {
        msg_apps_t msg;
        //????(?????)
        //msg.type = MSG_BTSTACK_CLEAR_TWSPAIR_LIST;    //??TWS????
        //msg.content.data[0] = 0;
        //??????
        //send_sync_msg_btmanager(NULL, &msg, NULL, 0);
        msg.type = MSG_BTSTACK_CLEAR_TWSPAIR_LIST; //?????????
        msg.content.data[0] = 0x02;   //0:清除手机配对信息；1:清除TWS配对信息2:清除所有配对信息
        //??????
        send_sync_msg_btmanager(NULL, &msg, NULL, 0);
    }
#endif
