...
extern int8 g_tws_discoverable_flag;
...
{       
    ...
    //PRINT_INFO_INT("conn_status", g_bt_stack_cur_info.conn_status);
        if((g_bt_stack_cur_info.dev_role==0)&&(g_bt_stack_cur_info.conn_status==CONN_STATUS_WAIT_PAIR))
        	
    {
        g_neednot_tts_play_timeout1=0;
        com_tts_state_play(TTS_MODE_ONLYONE,(void *) TTS_WAIT_BT_PAIR);
		set_led_state(LED_TWSPAIR);
		com_set_discoverable(3);
        ...
        if ((act_readl(GPIOBDAT) & (1<<9)) == 0)
        {
			...
			// PRINTD_INFO("UP SLAVE DISCOVERABLE3");
			 com_btmanager_conn_status_change_handle(CONN_STATUS_WAIT_PAIR);
			 g_tws_discoverable_flag = 0;   
			 
        }
		else
		{
             PRINTD_INFO("UP MASTER TWS");
