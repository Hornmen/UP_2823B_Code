...
void com_ap_switch_deal(app_result_e app_result)
{
    ...
    if (app_result == RESULT_SYSTEM_ENTER_S3)
    {
        ...
        // if((g_bt_stack_cur_info.num_connected == 0)||((g_bt_stack_cur_info.dev_role==TWS_MASTER)&&(g_bt_stack_cur_info.num_connected == 1)))
       // {
        //    //PRINT_INFO("time#W");   //不需要配置S3进入S4的时间  up-20170615
        //    uint32 tmp;
        //    tmp = act_readl(TIMER_CTL) &(~(0x7F << TIMER_CTL_S3BT_ON_TIMER_SHIFT));
        //    tmp |= (1 << TIMER_CTL_S3BT_ON_EN) |(1 << TIMER_CTL_S3BT_ON_TIMER_SHIFT);
         //   act_writel(tmp,TIMER_CTL);            
        //}
    //    else
        {            
            uint32 tmp;
            tmp = act_readl(TIMER_CTL) &(~(1 << TIMER_CTL_S3BT_ON_EN));
            act_writel(tmp,TIMER_CTL);
        }
    }
