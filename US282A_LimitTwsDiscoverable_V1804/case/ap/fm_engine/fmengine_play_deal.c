...
bool play(void)
{
    ...
    */
    ...
    result = mmm_pp_cmd(g_mmm_pp_handle, MMM_PP_PLAY,(unsigned int) &g_support_waves_fm);
#else
    //发送播放命令
...
#ifdef WAVES_ASET_TOOLS
    result = mmm_pp_cmd(g_mmm_pp_handle, MMM_PP_STOP, (unsigned int) &g_support_waves_fm);
#else
    //停止播放
