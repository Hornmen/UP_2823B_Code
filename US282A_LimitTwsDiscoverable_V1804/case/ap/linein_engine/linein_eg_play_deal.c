    ...
    if (g_eg_status_p->play_status == linein_play_status_stop)
    {
        ...
        */
	    ...
	    //mmm_pp_cmd(g_mmm_pp_handle, MMM_PP_SET_OUTPUT_CHANNEL, (unsigned int) OUTPUT_CHANNEL_NORMAL);
         ...
         //发送播放命令     
        result = mmm_pp_cmd(g_mmm_pp_handle, MMM_PP_PLAY, (unsigned int) &g_support_waves_linein);
#else
...
#ifdef WAVES_ASET_TOOLS
        //停止播放
        result = mmm_pp_cmd(g_mmm_pp_handle, MMM_PP_STOP, (unsigned int) &g_support_waves_linein);     
#else
        //停止播放
