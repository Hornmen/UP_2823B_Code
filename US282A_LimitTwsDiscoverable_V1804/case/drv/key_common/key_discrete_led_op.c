...
void __section__(".text.led_twinkle") _SetLEDOn_gpio(led_port_e port, uint8 pin)
{
    ...
    else if(LED_PORT_SPECIAL == port)
    {        
        act_writel(act_readl(SIO_OUTEN) | (uint32) (1 << pin), SIO_OUTEN);
        act_writel(act_readl(SIO_DAT) | (uint32) (1 << pin), SIO_DAT);
//        libc_print("\nSIO_OUTEN:",act_readl(SIO_OUTEN),2);
//        libc_print("SIO_DAT:",act_readl(SIO_DAT),2);
    }
    else
    {
#if (LED_ON_GPIO_SEL == LED_ON_GPIO_HIGH)
...
void __gpio_init(led_port_e port, uint8 pin)
{
    ...
    else if(port == LED_PORT_SPECIAL)
    {
        act_writel(0x00000000,DEBUGSIOIE);
        act_writel(0x00000000,DEBUGSIOOE);
        act_writel(act_readl(ADC_ANACTL) & (~(uint32) (1 << pin)), ADC_ANACTL);        
        if(pin == 2)
        {
