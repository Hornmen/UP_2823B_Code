...
#endif
        {
            if ((act_readb(BATADC_DATA) & 0x7f) < BAT_ADC_3_20_V)
            {
                enter_s4 = TRUE;
            }
        }
        ...
        welcome_argument_t *welcome_arg)
{
   ...
   /* uint32 wake_pd_onoff_val = 0;//唤醒onoff pending位
	uint32 onoff_state = 0;
	
	onoff_state = act_readl(ONOFF_KEY) & (1<<ONOFF_KEY_ONOFF_PRESS_0);
	
	//有onoff按键按下
	if ( onoff_state == (1<<ONOFF_KEY_ONOFF_PRESS_0))
	{
