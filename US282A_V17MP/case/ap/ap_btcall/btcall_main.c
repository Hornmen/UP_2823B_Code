...
int main(int argc, const char *argv[])
{
    ...
    //打电话前先备份当前音效的状态,打电话结束后好恢复
    sys_comval->bypass_temp = sys_comval->dae_cfg.bypass; 
    ...
    //恢复打电话前的音效状态
    if (FALSE == sys_comval->bypass_temp)     //打电话前音效是打开的状态
    {
        com_set_dae_onoff(TRUE);              //打开音效 
    }
    else if (TRUE == sys_comval->bypass_temp) //打电话前音效是关闭的状态
    {
        com_set_dae_onoff(FALSE);             //关闭音效
    }
