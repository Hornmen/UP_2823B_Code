...
app_result_e key_play_deal(void)
{
   ...
   //     PRINT_INFO(" g3");
    ...
    return RESULT_NULL;
#else
 /* if((g_bt_stack_cur_info.dev_role!=0)&&(g_bt_stack_cur_info.dev_role==
TWS_SLAVE)) //up modify 20170505
    {    
...
{
    #if 0
	bt_device_name_t bt_device_name; 
	
	bt_device_name.code_type = 0;
	// 注意 bt_device_name.name 一定需要'\0'结尾
	libc_memcpy(bt_device_name.name, "test alter name", 16);
	
	com_set_device_name(0, 0, &bt_device_name, 0);
	PRINT_INFO("NEXT D")	;
    #endif
#ifdef ENABLE_TRUE_WIRELESS_STEREO
    if (check_tws_tts_play() == FALSE)
