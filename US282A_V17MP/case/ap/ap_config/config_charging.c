...
app_result_e config_charging(int param)
{
    ...
    if((g_app_info_state_all.g_hp_led_state != LED_CHARGING) && (g_app_info_state_all.g_hp_led_state != LED_CHARGE_FULL))
    {
    ...
    {
        if(DC5V_MANAGER_MODE == 0)
        {
            result = RESULT_POWER_OFF;//充电线拔出就关机
        }
        else
        {
