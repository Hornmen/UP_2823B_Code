...
void en_play_wake_up(bool standby_enter)
{
	...
	//libc_print("ONOFF222",tmp,2);
    tmp = act_readl(WKEN_CTL);

	tmp  &= (~(1 << WKEN_CTL_SHORT_WKEN));
	tmp |= (1 << WKEN_CTL_LONG_WKEN); //play长按唤醒


   /* act_writel(act_readl(ONOFF_KEY) & (~(0x7<<ONOFF_KEY_ONOFF_PRESS_TIME_SHIFT)), ONOFF_KEY);
    sys_mdelay(1);
    if (FALSE == standby_enter)
    {
       
        act_writel(act_readl(ONOFF_KEY) | (0x5<<ONOFF_KEY_ONOFF_PRESS_TIME_SHIFT), ONOFF_KEY);
        sys_mdelay(1);
    }
    else
    {
   ...
   if (standby_enter == TRUE)
    ...
    config_flush_rtc_reg();
     //PRINT_INFO("123");
        ...
        tmp = act_readl(ONOFF_KEY);
		tmp &= ~ONOFF_KEY_ONOFF_PRESS_TIME_MASK;
		tmp |= 5<<ONOFF_KEY_ONOFF_PRESS_TIME_SHIFT;
		tmp |= 1<<ONOFF_KEY_ONOFF_RST_EN;
		tmp |= 0x30;
		act_writel(tmp,ONOFF_KEY);
		 config_flush_rtc_reg();
        //tmp = act_readl(ONOFF_KEY);
