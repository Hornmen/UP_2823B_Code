...
void disp_starting_check_poweron( void )
{
	bool enter_s4 = FALSE;
	uint8 sys_reboot = 0;
	...
	while (dt--)
	{
		...
		sys_reboot = (uint8) com_get_config_default(SETTING_CHARGE_FULL_DC5V_UNSTICK_SYS_MODE);
    	if(sys_reboot == 1)
    	{
			if(act_readl(RTC_BAK0) & (1<<MY_CHARGE_OUT_STATE))
			{
...
app_result_e config_poweron_option(void)
{
...
#endif
    {
		...
		if (config_detect_card() != -1) //UP MODIFY 20170602 FOR ATT update
		{	
