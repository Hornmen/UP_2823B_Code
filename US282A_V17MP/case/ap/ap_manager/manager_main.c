...
int main(int argc, const char *argv[])
{
    ...
    g_app_info_state_all.stub_pc_tools_type = 0xff;//初始化为0xff
    ...
    {
        if(argc ==WK_TYPE_S3BT_TIMERON)
        {
            param=6;
            //sys_exece_ap("config.ap", 0, 6);    //s3bt 唤醒进入s4状态
        }
        else
        {
            param=0;
    ...
    sys_exece_ap("config.ap", 0, param); 
    sys_os_sched_unlock();
