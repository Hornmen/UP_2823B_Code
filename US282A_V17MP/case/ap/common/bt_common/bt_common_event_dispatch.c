...
app_result_e com_btmanager_event_dispatch_bank(btstack_event_t *p_event, bool tts_flag)
{
    ...
    switch (msg_type)
    {
        ...
        case MSG_BTSTACK_ERR_CONNECTION_TIMEOUT:
        {
            ...
            else
            {
                ...
                g_neednot_tts_play_timeout = 1;
				  PRINTD_INFO("up call disconnect1");
                ...
                for (i = 0; i < g_btmanager_gl_var.support_dev_num; i++)
                {
                    if (g_bt_auto_connect_ctrl.dev_info[i].conn_flag == 0)
                    {
                        ...
                        {
                            PRINTD_INFO("up call disconnect");
                            g_neednot_tts_play_timeout1=1;
                            p_event->reserve[0]=0;
        ...
        case MSG_BTSTACK_DISCON_REASON:
        {
            libc_print("@dis_reason:",p_event->msg.content.data[0],2);
            g_btmanager_gl_var.dis_reason==p_event->msg.content.data[0];
        }
        break;

#endif
        case MSG_BTSTACK_DISCON_UNEXPECTED:
		PRINT_INFO("slave disconnect1111");	
