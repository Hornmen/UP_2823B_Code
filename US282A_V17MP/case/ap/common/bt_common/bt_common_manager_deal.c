      ...
      else
      {
         ...
         {      	
         	if (g_btmanager_gl_var.discoverable_flag == 1)
         	{
               ...
               #if 0
               //当单箱连接上手机，如果回连标志清0，那么需要关闭蓝牙的可连接性
               if(g_bt_stack_cur_info.rmt_dev[i].dev_type==NO_TWS_DEV)
               {
                    if((g_btmanager_gl_var.discoverable_flag!=2)&&(g_bt_auto_connect_ctrl.need_auto_connect==0)
                        &&((g_btmanager_gl_var.dis_reason!=0x08)||(g_btmanager_gl_var.dis_reason!=0x22)))
                    {
                        PRINT_INFO("#disable_con");
                        com_set_discoverable(2);
                        break;
                    }
