...
const sys_event_map_t __section__(".rodata.se_maplist") com_se_maplist[] =
{
    ...
    { { MSG_ADAPTOR_IN, SYSMSG_STOP_TTS_YES }, com_sys_deal_usb_in },
...
app_result_e com_sys_deal_usb_in(void *ev_param)
{
    app_result_e result = RESULT_POWER_OFF;
    
    *((REG32) (DAC_ANACTL)) &= (~(15 << DAC_ANACTL_LPANEN));//diff disable
    *((REG32) (DAC_ANACTL)) &= (~(1 << DAC_ANACTL_PAOSEN)); //pa stage
...
app_result_e com_sys_deal_lowpower(void *ev_param)
{
    ...
    else
    {
        ...
        set_led_state(LED_LOW_POWER_OFF);
...
app_result_e com_sys_deal_full_charge(void *ev_param)
{
	...
	if(g_app_info_state_all.g_hp_led_state != LED_CHARGE_FULL)
    {
        set_led_state(LED_CHARGE_FULL);
    }
