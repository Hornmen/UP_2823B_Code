...
const vol_hard_t g_hard_volume_table[VOLUME_VALUE_MAX+1] =
{
    ...
    {40, 0xa4}, {40, 0xa1}, {40, 0x9f}, {40, 0x9c}, {40, 0x99}, {40, 0x96}, {40, 0x90}, {40, 0x8c},
...
{
    /* 0db,     -0.375db,   -0.75db,    -1.5b,      -2.25db     -3db,       -3.75db,    -4.5db */
    {40, 0x96}, {40, 0x95}, {40, 0x94}, {40, 0x93}, {40, 0x91}, {40, 0x8f}, {40, 0x8d}, {40, 0x8b},
    /* -4.875db, -5.625db,   -6.375db,  -7.125db,   -7.5db      -8.25db,    -9db,       -9.375db */
    {40, 0x89}, {40, 0x86}, {40, 0x83}, {40, 0x80}, {40, 0x7d}, {40, 0x79}, {40, 0x75}, {40, 0x71},
    /* -10.125db, -11.25db,  -12db,      -13.125db, -14.25db    -15.75db,   -17.625db,  -19.125db */
    {40, 0x6d}, {40, 0x69}, {40, 0x65}, {40, 0x60}, {40, 0x5b}, {40, 0x56}, {40, 0x51}, {40, 0x4c},
    /* -21.375db,-23.625db,  -25.875db,  -28.125db, -31.5db     -35.625db,  -41.625db,      mute */
    {40, 0x46}, {40, 0x40}, {40, 0x3a}, {40, 0x33}, {40, 0x2c}, {40, 0x25}, {40, 0x1e}, {40, 0x00},
};
#endif

//调模拟音量300mv，耳机低摆幅输出500mv对应40  
const vol_hard_t g_hard_volume_table[VOLUME_VALUE_MAX+1] =
{
    /* 0db,         0db,    -0.375db,    -0.375db,   -0.75db     -0.75db,     -1.5db,    -1.5db */
   // {25, 0xbf}, {25, 0xbf}, {25, 0xbe}, {25, 0xbe}, {25, 0xbd}, {25, 0xbd}, {25, 0xbb}, {25, 0xbb},
    /* -2.25db,   -2.25db,     -3db,       -3db,     -5.625db,    -5.625db,  -7.125db,   -7.125db */
   // {25, 0xb9}, {25, 0xb9}, {25, 0xb7}, {25, 0xb7}, {25, 0xb0}, {25, 0xb0}, {25, 0xac}, {25, 0xac},
    /*-9.375db,  -9.375db,   -11.25db,   -11.25db,   -13.125db   -13.125db,  -15.75db,   -15.75db */
   // {25, 0xa6}, {25, 0xa6}, {25, 0xa1}, {25, 0xa1}, {25, 0x9c}, {25, 0x9c}, {25, 0x95}, {25, 0x95},
    /*-19.125db,-19.125db,  -23.625db,  -23.625db,   -31.5db     -31.5db ,     mute ,     mute */
   // {25, 0x8c}, {25, 0x8c}, {25, 0x80}, {25, 0x80}, {25, 0x6b}, {25, 0x6b}, {25, 0x00}, {25, 0x00},
     /* 0db,         0db,    -0.375db,    -0.375db,   -0.75db     -0.75db,     -1.5db,    -1.5db */
     /* 0db,         0db,    -0.375db,    -0.375db,   -0.75db     -0.75db,     -1.5db,    -1.5db */
    {28, 0xbf}, {28, 0xbf}, {28, 0xbe}, {28, 0xbe}, {28, 0xbd}, {28, 0xbd}, {28, 0xbb}, {28, 0xbb},//up modify 20170417 for volume increase
    /* -2.25db,   -2.25db,     -3db,       -3db,     -5.625db,    -5.625db,  -7.125db,   -7.125db */
    {28, 0xb9}, {28, 0xb9}, {28, 0xb7}, {28, 0xb7}, {28, 0xb0}, {28, 0xb0}, {28, 0xac}, {28, 0xac},
    /*-9.375db,  -9.375db,   -11.25db,   -11.25db,   -13.125db   -13.125db,  -15.75db,   -15.75db */
    {28, 0xa6}, {28, 0xa6}, {28, 0xa1}, {28, 0xa1}, {28, 0x9c}, {28, 0x9c}, {28, 0x95}, {28, 0x95},
    /*-19.125db,-19.125db,  -23.625db,  -23.625db,   -31.5db     -31.5db ,     mute ,     mute */
    {28, 0x8c}, {28, 0x8c}, {28, 0x80}, {28, 0x80}, {28, 0x6b}, {28, 0x6b}, {28, 0x00}, {28, 0x00},
