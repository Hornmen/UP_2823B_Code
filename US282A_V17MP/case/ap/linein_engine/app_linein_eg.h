...
typedef struct
{
    uint8 pc_tools:2;    //0x00:不支持WAVES音效; 0x01:WAVES音效调试模式; 0x02:WAVES音效固件模式
    uint8 w_input:1;     //0x00:WAVES音效固件模式下不支持input参数;0x01:WAVES音效固件模式下支持input参数;
    uint8 reserve:5;        
} support_waves_linein_t;

extern  support_waves_linein_t g_support_waves_linein;
#endif
