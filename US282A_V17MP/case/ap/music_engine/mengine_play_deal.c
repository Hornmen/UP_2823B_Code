...
bool play(play_mode_e play_mode)
{
    ...
    if (play_mode == PLAY_FFON)
    {
        ...
        result = mmm_mp_cmd(g_mp_handle, MMM_MP_PLAY, (unsigned int) &g_support_waves_music);
#else
...
bool stop(stop_mode_e stop_mode)
{
    ...
    if ((g_eg_status_p->play_status == PlaySta) || (g_eg_status_p->play_status == PlayFast))
    {
        ...
        //停止播放
#ifdef WAVES_ASET_TOOLS
        result = mmm_mp_cmd(g_mp_handle, MMM_MP_STOP, (unsigned int) &g_support_waves_music);
#else      
