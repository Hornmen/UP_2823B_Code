...
//用大电池和稳压源为准结果1-30
//低电电压
#define BATLOW_VEL      BAT_ADC_3_20_V
//充满电电压
...
const uint8 __section__(".rodata_1") battery_grade_vel[BATTERY_GRADE_MAX+1] =
{
    ...
    BAT_ADC_3_30_V, //一格电  返回 2
    BAT_ADC_3_40_V, //一格电  返回 3
    BAT_ADC_3_50_V, //两格电  返回 4
    BAT_ADC_3_60_V, //两格电  返回 5
    BAT_ADC_3_70_V, //三格电  返回 6
    BAT_ADC_3_80_V, //三格电  返回 7
    BAT_ADC_3_90_V, //四格电  返回 8
    BAT_ADC_4_00_V  //四格电  返回 9
};
 ...
 * \note
 *******************************************************************************/
static int32 __section__(".bank1") key_get_volume_limit(uint8 batadc)
{
    int32 i;
 ...
 *    Wekan   2015-3-26am         1.0             review this
 *******************************************************************************/
battery_status_e __section__(".bank1") key_inner_battery_get_status(uint32 *ad_val, int32* vol_limit, uint8* app_id)
{ 
    ...
    //libc_print("bat:",cur_battery_val,2);
    for (i = 0; i < BAT_ADC_DATA_NUM; i++)
    {
