...
//0x9fc3afe0-9fc3afff(32字节)地址存放PC工具发来的系数表
#define WAVES_ASET_COEFF_ADDR                     (0x9fc3afe0)//(dsp地址:0x2d7f0)
//M4K和DSP协议交互结构体
typedef struct
{
    int DAE_change_flag;            //1表示更新了音效系数;!!!注意M4K置位，DSP取走参数后复位
    int parameter_address;          //更新的音效系数长度
    int parameter_length;           //更新音效系数存放在内存中的起始地址
    int reserve[5];                 //扩展用
}coeff_t;

//0x9fc3b000-0x9fc3bfff(4k空间用来作为工具和小机数据交互的缓冲buffer，现在只用了256个字节，剩余的空间可以用其它用途)
#define WAVES_ASET_SHARE_MEM_START_ADDR           (0x9fc3b000)//(dsp地址:0x2d800)
#define WAVES_ASET_SHARE_MEM_END_ADDR             (0x9fc3bfff)//(dsp地址:0x2dfff)
/******************************************************************/



/*****从0x9fc37000起始地址后的1616字节的数据存放的是WAVES音效参数*********************/
...
//waves音效input参数起始地址
#define WAVES_INPUT_PARA_START_ADDR               (0x2b800 + 4)//(mips地址:0x9fc37008) 
